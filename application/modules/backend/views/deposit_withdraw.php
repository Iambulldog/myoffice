        ...
        <form  method="post" action="backend/addlist" id="f_list">
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-4">
                    ...
                    <select id="s_web" name="s_web" class=" selectpicker" data-live-search="true" required onchange="changeweb(this.value);">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="mb-0 table table-hover" id="tb_all" width="100%" cellspacing="0">
                    ...
                    <tbody>
                    <?php $i=1;  foreach ($list_all as $k => $val) {?>
                    <tr style ="<?php if($val->list==1){/*ฝาก*/echo "background-color: #b1ffe255;";}if($val->list==2){/*ถอน*/echo " background-color: #fdacac55;";}if($val->status==0) {
                       echo "background-color: #00000033;";
                    }?>">
                            <td> <?=$i?> </td>
                            <td> <?php if($val->list==1){echo "ฝาก";}elseif($val->list==2){echo "ถอน";}?> </td>
                            <td> <?=$val->web_name?> </td>
                            <td> <?=$val->name_user?> </td>
                            <td> <?php if($val->list==1){echo "+";}elseif($val->list==2){echo "-";}?><?=$val->total?> </td>
                            <td> <?=$val->bonus?> </td>
                            ...
                            ?>
                                        ...
                                        <i class="fas fa-fw fa-trash remove" data-id="<?= $val->id;?>"  data-placement="top" title="ลบ"></i>|

                                <?php }else{?>

                                    <i class="fas fa-fw fa-recycle recycle" data-id="<?= $val->id;?>"  data-placement="top" title="กู้คืน"></i>
...
$(".recycle").click(function() {
        ...
        $.ajax({
                    ...
                    success: function(d) {
                        ...
                        $.each(d, function(key, val) {
                            var list=""; if(val.list==1){list= "ฝาก";}else if(val.list==2){list= "ถอน";}
                             t.row.add( [
                                        c,
                                        list,
                                        val.web_name,
    ...
    function check_f() {
    if($("#s_list").val() != 0 && $("#s_web").val() != 0 && $("#s_user").val() != 0 && $("#total").val() != ""){
